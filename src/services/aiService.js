...
class AIService {
  ...
  async generateEventDescription(eventDetails) {
    try {
      ...
      throw new Error(error.response?.data?.message || 'Failed to generate event description');
    }
  }
  ...
  async getEventPlanningAdvice(eventType, requirements) {
    try {
      ...
      throw new Error(error.response?.data?.message || 'Failed to get planning advice');
    }
  }
